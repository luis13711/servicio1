...
namespace servicio1
{
    public partial class Service1 : ServiceBase
    {
        ...
        private static void OnTimedEvent(Object source, System.Timers.ElapsedEventArgs e)
        {
            try
            {
                //StreamWriter WriteReportFile = File.AppendText(@"C:\mvFiles\SQLReport.txt");
                string line = $"{DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss")}";
                string textopath = Application.StartupPath + "\\tramas.txt";
                using (System.IO.StreamWriter file = File.AppendText(textopath))//new System.IO.StreamWriter(textopath))
                {
                    file.WriteLine(line);
                }
        ...
        //    string line =$"{DateTime.Now.ToString("dd/MM/yyyy")}";
